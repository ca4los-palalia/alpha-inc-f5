...
public class ControlVM extends ControlMetaclass {
	...
	public void buscarOrdenCompra() {
		if ((this.checkBuscarNueva) || (this.checkBuscarCancelada) || (this.checkBuscarEnviada)
			...
			this.ordenesCompras = this.ordenCompraService
					.getOrdenesByEstatusAndFolioOr(this.requisicion.getBuscarRequisicion(), listaEstatus);
	...
	public void mostrarProductosOrdenCompra() {
		...
		if (this.cotizacionSelected != null) {
			...
			for (Cotizacion item : this.cotizacionesConProductos) {
				...
				this.precioTotal = Float.valueOf(this.precioTotal.floatValue()
	...
	public void abrirVentanaCancelacion() {
		if (this.ordenCompra != null) {
			if (this.ordenCompra.getEstatusRequisicion().getClave().equals("OCN")) {
				...
				StockUtils.showSuccessmessage(
						"La cotizacion con folio -" + this.ordenCompra.getCodigo()
								+ "- nu puede ser cancelada bajo este estatus ("
								+ this.ordenCompra.getEstatusRequisicion().getNombre() + ")",
						"warning", Integer.valueOf(0), null);
			}
		} else {
				...
				for (Privilegios privilegio : privilegios) {
					if (privilegio.getUsuarios().getPersona().getContacto() != null) {
						this.mailService.sendMail(
								...
								"La cotizacion " + this.ordenCompra.getCodigo() + " ha sido aceptada");
					}
				}
				StockUtils.showSuccessmessage(
						"La orden de compra [" + this.ordenCompra.getCodigo() + "] ha sido Aceptada", "info",
						Integer.valueOf(0), null);
			} else {
				StockUtils.showSuccessmessage(
						"La orden de compra [" + this.ordenCompra.getCodigo()
	...
	public void imprimirOrdenCompra() {
		if (this.ordenCompra != null) {
			this.cotizacionSelected = this.ordenCompra.getCotizacion();
			if ((this.cotizacionSelected != null) && (this.cotizacionesConProductos != null)
					&& (this.cotizacionesConProductos.size() > 0)) {
				...
				StockUtils.showSuccessmessage(
						generarOrdenCompraJasper(listaHashsParametros, aplicaciones, this.cotizacionesConProductos),
				...
				((Cotizacion) this.cotizacionesConProductos.get(0)).getRequisicionProducto().getCofiaPartidaGenerica()
						.getNombre();

				((Cotizacion) this.cotizacionesConProductos.get(0)).getRequisicionProducto().getCantidad();

				((Cotizacion) this.cotizacionesConProductos.get(0)).getRequisicionProducto()
	...
	public void showModal(Event e) {
		...
		inputParams.put("listaKey", this.cotizacionesConProductos);

		inputParams.put("areasKey", this.areas);
